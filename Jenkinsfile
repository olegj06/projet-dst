...
stages {
        ...
        stage('Docker run'){ // run container from our builded image
                steps {
                    script {
                    sh '''
                    docker run -d -p 8088:80 --name wordpress Docker/wordpress/$DOCKER_ID/$DOCKER_IMAGE1:$DOCKER_TAG
                    docker run -d -p 3306:3306 --name mariadb Docker/mariadb/$DOCKER_ID/$DOCKER_IMAGE2:$DOCKER_TAG
                    sleep 10
                    '''
                    }
                }
